from datetime import timedelta
from unittest.mock import patch

from django.core.cache import cache
from django.test import TestCase
from django.utils import timezone

from ech.analytics.exceptions import AnalyticsProductUnavailableException
from ech.analytics.models import AnalyticsSnapshot
from ech.analytics.services.analytic_product_performance_service import (
    AnalyticsProductPerformanceService,
)


class AnalyticsProductPerformanceServiceTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.period_start = timezone.now().replace(
            hour=0,
            minute=0,
            second=0,
            microsecond=0,
        )
        cls.period_end = cls.period_start + timedelta(days=1)

    def setUp(self):
        cache.clear()

    def _build_realtime_payload(self):
        return {
            "source": "realtime",
            "snapshot_id": None,
            "period_type": None,
            "period_start": self.period_start,
            "period_end": self.period_end,
            "products_sold": 15,
            "top_product_id": 123,
        }

    def test_get_performance_returns_snapshot_data_when_available(self):
        snapshot = AnalyticsSnapshot(
            id=601,
            period_type=AnalyticsSnapshot.PERIOD_DAILY,
            period_start=self.period_start,
            period_end=self.period_end,
            products_sold=15,
            top_product_id=123,
        )

        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "get_latest_analytics_snapshot_by_period_type",
            return_value=snapshot,
        ), patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsLogService.log_product_metrics_calculated"
        ) as log_mock:
            result = AnalyticsProductPerformanceService.get_performance(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result["source"], "snapshot")
        self.assertEqual(result["snapshot_id"], 601)
        self.assertEqual(result["products_sold"], 15)
        self.assertEqual(result["top_product_id"], 123)

        log_mock.assert_called_once_with(
            period_start=self.period_start,
            period_end=self.period_end,
            products_sold=15,
            top_product_id=123,
            performed_by=None,
        )

    def test_get_performance_uses_realtime_when_snapshot_mismatch(self):
        mismatched_snapshot = AnalyticsSnapshot(
            id=602,
            period_type=AnalyticsSnapshot.PERIOD_DAILY,
            period_start=self.period_start - timedelta(days=1),
            period_end=self.period_end - timedelta(days=1),
        )

        realtime_payload = self._build_realtime_payload()

        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "get_latest_analytics_snapshot_by_period_type",
            return_value=mismatched_snapshot,
        ), patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsProductPerformanceService._build_performance_realtime",
            return_value=realtime_payload,
        ) as realtime_mock, patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsLogService.log_product_metrics_calculated"
        ):
            result = AnalyticsProductPerformanceService.get_performance(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result["source"], "realtime")
        self.assertEqual(result["products_sold"], 15)
        self.assertEqual(result["top_product_id"], 123)

        realtime_mock.assert_called_once_with(
            period_start=self.period_start,
            period_end=self.period_end,
        )

    def test_get_performance_uses_cache(self):
        realtime_payload = self._build_realtime_payload()

        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "get_latest_analytics_snapshot_by_period_type",
            side_effect=Exception("no snapshot"),
        ), patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsProductPerformanceService._build_performance_realtime",
            return_value=realtime_payload,
        ) as realtime_mock, patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsLogService.log_product_metrics_calculated"
        ):
            result_1 = AnalyticsProductPerformanceService.get_performance(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

            result_2 = AnalyticsProductPerformanceService.get_performance(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result_1, result_2)
        realtime_mock.assert_called_once()

    def test_get_performance_rebuilds_after_cache_clear(self):
        realtime_payload = self._build_realtime_payload()

        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "get_latest_analytics_snapshot_by_period_type",
            side_effect=Exception("no snapshot"),
        ), patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsProductPerformanceService._build_performance_realtime",
            return_value=realtime_payload,
        ) as realtime_mock, patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsLogService.log_product_metrics_calculated"
        ):
            AnalyticsProductPerformanceService.get_performance(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

            cache.clear()

            AnalyticsProductPerformanceService.get_performance(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(realtime_mock.call_count, 2)

    def test_get_performance_handles_empty_metrics(self):
        empty_payload = {
            "source": "realtime",
            "snapshot_id": None,
            "period_type": None,
            "period_start": self.period_start,
            "period_end": self.period_end,
            "products_sold": 0,
            "top_product_id": None,
        }

        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "get_latest_analytics_snapshot_by_period_type",
            side_effect=Exception("no snapshot"),
        ), patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsProductPerformanceService._build_performance_realtime",
            return_value=empty_payload,
        ), patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsLogService.log_product_metrics_calculated"
        ):
            result = AnalyticsProductPerformanceService.get_performance(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result["products_sold"], 0)
        self.assertIsNone(result["top_product_id"])

    def test_get_performance_raises_when_cache_layer_fails(self):
        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "AnalyticsCacheService.get_or_set",
            side_effect=Exception("cache failure"),
        ):
            with self.assertRaises(AnalyticsProductUnavailableException):
                AnalyticsProductPerformanceService.get_performance(
                    period_type=AnalyticsSnapshot.PERIOD_DAILY,
                    period_start=self.period_start,
                    period_end=self.period_end,
                )

    def test_resolve_period_bounds_raises_when_only_one_bound_is_provided(self):
        with self.assertRaises(AnalyticsProductUnavailableException):
            AnalyticsProductPerformanceService._resolve_period_bounds(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=None,
            )

    def test_get_matching_snapshot_returns_none_when_selector_raises(self):
        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "get_latest_analytics_snapshot_by_period_type",
            side_effect=Exception("not found"),
        ):
            result = AnalyticsProductPerformanceService._get_matching_snapshot(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertIsNone(result)

    def test_get_matching_snapshot_returns_none_when_bounds_do_not_match(self):
        snapshot = AnalyticsSnapshot(
            id=777,
            period_type=AnalyticsSnapshot.PERIOD_DAILY,
            period_start=self.period_start - timedelta(days=2),
            period_end=self.period_end - timedelta(days=2),
        )

        with patch(
            "ech.analytics.services.analytic_product_performance_service."
            "get_latest_analytics_snapshot_by_period_type",
            return_value=snapshot,
        ):
            result = AnalyticsProductPerformanceService._get_matching_snapshot(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertIsNone(result)

    def test_build_performance_from_snapshot_handles_empty_values(self):
        snapshot = AnalyticsSnapshot(
            id=701,
            period_type=AnalyticsSnapshot.PERIOD_DAILY,
            period_start=self.period_start,
            period_end=self.period_end,
            products_sold=0,
            top_product_id=None,
        )

        result = AnalyticsProductPerformanceService._build_performance_from_snapshot(
            snapshot=snapshot,
        )

        self.assertEqual(result["source"], "snapshot")
        self.assertEqual(result["snapshot_id"], 701)
        self.assertEqual(result["products_sold"], 0)
        self.assertIsNone(result["top_product_id"])