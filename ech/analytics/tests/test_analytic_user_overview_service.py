from datetime import timedelta
from unittest.mock import patch

from django.core.cache import cache
from django.test import TestCase
from django.utils import timezone

from ech.analytics.exceptions import AnalyticsUserUnavailableException
from ech.analytics.models import AnalyticsSnapshot
from ech.analytics.services.analytic_user_overview_service import (
    AnalyticsUserOverviewService,
)


class AnalyticsUserOverviewServiceTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.period_start = timezone.now().replace(
            hour=0,
            minute=0,
            second=0,
            microsecond=0,
        )
        cls.period_end = cls.period_start + timedelta(days=1)

    def setUp(self):
        cache.clear()

    def _build_realtime_payload(self):
        return {
            "source": "realtime",
            "snapshot_id": None,
            "period_type": None,
            "period_start": self.period_start,
            "period_end": self.period_end,
            "total_registered_users": 100,
            "active_users": 75,
            "inactive_users": 25,
            "confirmed_users": 80,
            "unconfirmed_users": 20,
            "staff_users": 10,
            "customer_users": 90,
        }

    def test_get_overview_returns_snapshot_data_when_available(self):
        snapshot = AnalyticsSnapshot(
            id=901,
            period_type=AnalyticsSnapshot.PERIOD_DAILY,
            period_start=self.period_start,
            period_end=self.period_end,
            total_registered_users=100,
            active_users=75,
            inactive_users=25,
            confirmed_users=80,
            unconfirmed_users=20,
            staff_users=10,
            customer_users=90,
        )

        with patch(
            "ech.analytics.services.analytic_user_overview_service."
            "get_latest_analytics_snapshot_by_period_type",
            return_value=snapshot,
        ), patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsLogService.log_user_metrics_calculated"
        ) as log_mock:
            result = AnalyticsUserOverviewService.get_overview(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result["source"], "snapshot")
        self.assertEqual(result["snapshot_id"], 901)
        self.assertEqual(result["total_registered_users"], 100)
        self.assertEqual(result["active_users"], 75)
        self.assertEqual(result["inactive_users"], 25)
        self.assertEqual(result["confirmed_users"], 80)
        self.assertEqual(result["unconfirmed_users"], 20)
        self.assertEqual(result["staff_users"], 10)
        self.assertEqual(result["customer_users"], 90)

        log_mock.assert_called_once_with(
            period_start=self.period_start,
            period_end=self.period_end,
            total_registered_users=100,
            active_users=75,
            performed_by=None,
        )

    def test_get_overview_uses_realtime_when_snapshot_mismatch(self):
        mismatched_snapshot = AnalyticsSnapshot(
            id=902,
            period_type=AnalyticsSnapshot.PERIOD_DAILY,
            period_start=self.period_start - timedelta(days=1),
            period_end=self.period_end - timedelta(days=1),
        )

        realtime_payload = self._build_realtime_payload()

        with patch(
            "ech.analytics.services.analytic_user_overview_service."
            "get_latest_analytics_snapshot_by_period_type",
            return_value=mismatched_snapshot,
        ), patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsUserOverviewService._build_overview_realtime",
            return_value=realtime_payload,
        ) as realtime_mock, patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsLogService.log_user_metrics_calculated"
        ):
            result = AnalyticsUserOverviewService.get_overview(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result["source"], "realtime")
        self.assertEqual(result["total_registered_users"], 100)
        self.assertEqual(result["active_users"], 75)
        self.assertEqual(result["inactive_users"], 25)
        self.assertEqual(result["confirmed_users"], 80)
        self.assertEqual(result["unconfirmed_users"], 20)
        self.assertEqual(result["staff_users"], 10)
        self.assertEqual(result["customer_users"], 90)

        realtime_mock.assert_called_once()

    def test_get_overview_uses_cache(self):
        realtime_payload = self._build_realtime_payload()

        with patch(
            "ech.analytics.services.analytic_user_overview_service."
            "get_latest_analytics_snapshot_by_period_type",
            side_effect=Exception("no snapshot"),
        ), patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsUserOverviewService._build_overview_realtime",
            return_value=realtime_payload,
        ) as realtime_mock, patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsLogService.log_user_metrics_calculated"
        ):
            result_1 = AnalyticsUserOverviewService.get_overview(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

            result_2 = AnalyticsUserOverviewService.get_overview(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result_1, result_2)
        realtime_mock.assert_called_once()

    def test_get_overview_rebuilds_after_cache_clear(self):
        realtime_payload = self._build_realtime_payload()

        with patch(
            "ech.analytics.services.analytic_user_overview_service."
            "get_latest_analytics_snapshot_by_period_type",
            side_effect=Exception("no snapshot"),
        ), patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsUserOverviewService._build_overview_realtime",
            return_value=realtime_payload,
        ) as realtime_mock, patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsLogService.log_user_metrics_calculated"
        ):
            AnalyticsUserOverviewService.get_overview(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

            cache.clear()

            AnalyticsUserOverviewService.get_overview(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(realtime_mock.call_count, 2)

    def test_get_overview_handles_empty_metrics(self):
        empty_payload = {
            "source": "realtime",
            "snapshot_id": None,
            "period_type": None,
            "period_start": self.period_start,
            "period_end": self.period_end,
            "total_registered_users": 0,
            "active_users": 0,
            "inactive_users": 0,
            "confirmed_users": 0,
            "unconfirmed_users": 0,
            "staff_users": 0,
            "customer_users": 0,
        }

        with patch(
            "ech.analytics.services.analytic_user_overview_service."
            "get_latest_analytics_snapshot_by_period_type",
            side_effect=Exception("no snapshot"),
        ), patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsUserOverviewService._build_overview_realtime",
            return_value=empty_payload,
        ), patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsLogService.log_user_metrics_calculated"
        ):
            result = AnalyticsUserOverviewService.get_overview(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=self.period_end,
            )

        self.assertEqual(result["total_registered_users"], 0)
        self.assertEqual(result["active_users"], 0)
        self.assertEqual(result["inactive_users"], 0)
        self.assertEqual(result["confirmed_users"], 0)
        self.assertEqual(result["unconfirmed_users"], 0)
        self.assertEqual(result["staff_users"], 0)
        self.assertEqual(result["customer_users"], 0)

    def test_get_overview_raises_when_cache_layer_fails(self):
        with patch(
            "ech.analytics.services.analytic_user_overview_service."
            "AnalyticsCacheService.get_or_set",
            side_effect=Exception("cache failure"),
        ):
            with self.assertRaises(AnalyticsUserUnavailableException):
                AnalyticsUserOverviewService.get_overview(
                    period_type=AnalyticsSnapshot.PERIOD_DAILY,
                    period_start=self.period_start,
                    period_end=self.period_end,
                )

    def test_resolve_period_bounds_raises_when_only_one_bound_is_provided(self):
        with self.assertRaises(AnalyticsUserUnavailableException):
            AnalyticsUserOverviewService._resolve_period_bounds(
                period_type=AnalyticsSnapshot.PERIOD_DAILY,
                period_start=self.period_start,
                period_end=None,
            )