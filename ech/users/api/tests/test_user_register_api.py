...
class UserRegisterApiTestCase(APITestCase):
    ...
    def test_register_user_success(self):
        ...
        self.assertEqual(user.user_email, data["email"])
    ...
    def test_register_user_with_same_idempotency_key_returns_same_user(self):
        ...
        self.assertEqual(
            first_response.data["user_name"],
            second_response.data["user_name"],
        )
    ...
    def test_register_user_with_same_idempotency_key_and_different_payload_returns_conflict(
        self,
    ):
        """
        Reused idempotency key with a different payload must return 409.
        """
        ...
        self.assertEqual(
            User.objects.filter(idempotency_key=idempotency_key).count(),
            1,
        )
    ...
    def test_register_user_with_missing_required_fields_returns_400(self):
        """
