...
class OrderStatusService:
    ...
    def confirm_order(self):
        ...
        if self.order.status != Order.ORDER_STATUS_PENDING:
            raise ValueError(MSG_ERROR_ONLY_PENDING_ORDERS_CAN_BE_CONFIRMED)
        ...
        with transaction.atomic():
            ...
            lifecycle.confirmed_at = timezone.now()
    ...
    def start_processing(self):
        ...
        if self.order.status != Order.ORDER_STATUS_CONFIRMED:
            raise ValueError(MSG_ERROR_ORDERS_MUST_BE_CONFIRMED)
        ...
        with transaction.atomic():
            ...
            lifecycle = self.order.lifecycle
            lifecycle.processing_at = timezone.now()
    ...
    def ship_order(self):
        ...
        with transaction.atomic():
            ...
            if self.order.status != Order.ORDER_STATUS_PROCESSING:
                raise ValueError(MSG_ERROR_ORDERS_MUST_BE_PROCESSING)
    ...
    def deliver_order(self):
        ...
        if self.order.status != Order.ORDER_STATUS_SHIPPED:
            raise ValueError(MSG_ERROR_ORDERS_MUST_BE_SHIPPED)
        ...
        with transaction.atomic():
            ...
            lifecycle = self.order.lifecycle
            lifecycle.delivered_at = timezone.now()
    ...
    def cancel_order(self):
        ...
        with transaction.atomic():
            ...
            lifecycle = self.order.lifecycle
            lifecycle.cancelled_at = timezone.now()
            lifecycle.save(update_fields=["cancelled_at"])

            self._register_event(OrderEvent.TYPE_CANCELLED)
