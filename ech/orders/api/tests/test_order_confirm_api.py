...
class OrderConfirmApiTestCase(APITestCase):
    def setUp(self):
        cache.clear()
    ...
    def test_confirm_order_successfully(self, mock_confirm_order):
        ...
        def side_effect():
            ...
            order.payment_status = Order.PAYMENT_STATUS_AUTHORIZED
            order.shipping_status = Order.SHIPPING_STATUS_PREPARING
            order.save(update_fields=["status", "payment_status", "shipping_status"])

            order.lifecycle.confirmed_at = timezone.now()
            order.lifecycle.save(update_fields=["confirmed_at", "updated_at"])

            invalidate_order_related_caches(order)

        mock_confirm_order.side_effect = side_effect
        ...
        self.assertEqual(response.data["status"], Order.ORDER_STATUS_CONFIRMED)
        self.assertEqual(response.data["payment_status"], Order.PAYMENT_STATUS_AUTHORIZED)
        self.assertEqual(response.data["shipping_status"], Order.SHIPPING_STATUS_PREPARING)
        self.assertIn("lifecycle", response.data)
        self.assertIsNotNone(response.data["lifecycle"]["confirmed_at"])
    ...
    def test_confirm_order_returns_400_when_service_raises_order_error(self, mock_confirm_order):
        ...
        mock_confirm_order.side_effect = OrderError("Business error")
